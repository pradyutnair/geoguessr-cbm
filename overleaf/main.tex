...
\begin{abstract}
Image geolocation remains challenging due to visual ambiguity across regions. Existing approaches achieve reasonable accuracy but operate as black boxes, providing no insight into \textit{why} a location is predicted. We propose a concept-aware geolocation system using hierarchical concept bottleneck models that ensure predictions flow through interpretable semantic concepts. Our three-stage pipeline combines contrastive pretraining, text-anchored concept learning, and gated fusion for coordinate prediction. On a 43,000-image street view dataset, we achieve 126~km median error on in-distribution data and 350~km on out-of-distribution GeoGuessr images, while providing patch-level attention maps revealing which visual regions support concept-based predictions. Our code is available at \url{https://github.com/pradyutnair/geoguessr-cbm}.
\end{abstract}
...
To quantify the relative importance of semantic concepts versus visual features, we perform controlled ablation by shuffling inputs across the batch during inference. Specifically, we evaluate three conditions: (1) \textit{baseline}, where concept embeddings and patch tokens are correctly paired with their source images; (2) \textit{concept shuffled}, where concept embeddings are randomly permuted across the batch while patch tokens remain aligned to their original images; and (3) \textit{patches shuffled}, where patch tokens are permuted while concept embeddings remain aligned. This design isolates each modality's contribution: if shuffling a modality causes large degradation, the model relies heavily on proper alignment of that modality for accurate predictions.

Table~\ref{tab:swap_shuffle} presents the results on the validation set. Shuffling concept embeddings increases median error from 28.6~km to 8340.5~km, a 291$\times$ degradation reducing predictions to near-random guessing. In contrast, shuffling patch tokens causes only 1.35$\times$ degradation to 38.6~km. This asymmetry confirms that learned semantic concepts serve as the primary signal for geolocation, encoding geographic information through text-anchored prototypes for features like script type, road signs, vegetation, and architectural style. Cross-attention over image patches provides complementary spatial refinement ($\sim$10~km), attending to fine-grained visual details that disambiguate nearby locations sharing similar semantic properties.
...
\paragraph{Qualitative Interpretability.}
Figures~\ref{fig:interpretability_example_b} and~\ref{fig:interpretability_example_a} (Appendix~\ref{app:interpretability}) present per-image interpretability outputs: input panorama, patch attention heatmaps, geographic predictions with cell assignments, gate values, and hierarchical concept predictions. The model identifies scene-level attributes correlating with geographic regions (e.g., ``script\_cyrillic'' for Russian locations). Attention concentrates on semantically relevant regions such as road surfaces, signage, vegetation, and architecture. More examples are available in our code repository.

\paragraph{Gate Value Dynamics.}
Across the validation set, gate values have mean 0.44 (near-balanced), standard deviation 0.27, and span the full range [0.01, 0.99], confirming the model leverages both information sources rather than collapsing to a single modality. Figure~\ref{fig:gate_distribution} shows both variants converge to high 90th-percentile values (0.83--0.86), with vanilla showing slightly higher concept reliance than finetuned, which leverages both sources more effectively through learned spatial representations.
...
Future directions include: (1) expanding the dataset for improved concept coverage and finer-grained geolocation, (2) automatic concept discovery to reduce reliance on curated vocabularies, and (3) integration with retrieval-based methods for hybrid prediction.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{round_01_110135_concepts.png}
    \caption{Interpretability analysis for a European scene. The patch attention heatmap reveals focus on road geometry and built structures. Concept predictions capture semantic categories relevant to the predicted region, with balanced concept-image fusion indicated by the gate value.}
    \label{fig:interpretability_example_b}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{round_09_110825_concepts.png}
    \caption{Interpretability analysis for a West African scene. The model identifies this location through distinctive visual cues including unique vehicles and license plates (gate 0.431). Top child concepts include ``Unique car'' and ``License Plates''; parent concepts indicate ``car\_meta'' and ``landscape\_savannah'' categories.}
