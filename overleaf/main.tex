...
\begin{figure}[H]
    ...
    \begin{tabularx}{\textwidth}{YYY}
        ...
        \small Czechia: Distinctive bollard &
...
Table~\ref{tab:swap_shuffle} presents the results on the validation set. Shuffling concept embeddings increases median error from 28.6 km to 8340.5 km, a catastrophic 291$\times$ degradation that reduces predictions to near-random guessing across the globe. In contrast, shuffling patch tokens causes only a 1.35$\times$ degradation to 38.6 km. This asymmetry confirms that the learned semantic concepts from Stage 1 serve as the primary signal for geolocation, encoding geographic information through text-anchored prototypes for features like script type, road signs, vegetation, and architectural style. The cross-attention mechanism over image patches provides complementary spatial refinement of approximately 10 km, likely by attending to fine-grained visual details that disambiguate between nearby locations sharing similar semantic properties.
...
Figures~\ref{fig:interpretability_example_b} and~\ref{fig:interpretability_example_a} in Appendix~\ref{app:interpretability} present per-image interpretability outputs including: the input panorama, patch attention heatmaps showing attended regions, geographic predictions with cell assignments, gate values indicating concept-image balance, and hierarchical concept predictions at both child and parent levels.

The visualizations reveal that the model correctly identifies scene-level attributes correlating with geographic regions (e.g., ``script\_cyrillic'' for Russian locations, ``landscape\_arctic'' for Scandinavian scenes). Attention concentrates on semantically relevant regions such as road surfaces, signage, vegetation, and architecture. Parent concepts (e.g., ``car\_meta'', ``road\_sign'') provide coarse semantic grounding while child concepts enable fine-grained geographic discrimination.

\paragraph{Gate Value Dynamics.}
The learned gating mechanism dynamically balances concept and image contributions. Across the validation set, gate values have mean 0.44 (near-balanced, slightly image-biased), standard deviation 0.27 (high variance indicating adaptive behavior), and span the full range [0.01, 0.99]. This confirms the model leverages both information sources rather than collapsing to a single modality, adapting based on image content.

Figure~\ref{fig:gate_distribution} shows gate value evolution during training. Both vanilla and finetuned variants converge to high 90th-percentile values (0.83--0.86), indicating concept information dominates upper embedding dimensions. The vanilla model shows slightly higher gate values, suggesting greater reliance on semantic concepts without Stage 0 pretraining, while the finetuned model leverages both sources more effectively through learned spatial representations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{val_gate.png}
    \caption{90th percentile of learned gate values during validation. Both models converge to high values (0.83--0.86), with vanilla showing slightly higher concept reliance than finetuned.}
