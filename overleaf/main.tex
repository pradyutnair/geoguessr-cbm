...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.95\textwidth]{report/interpretability/example_norway_residential.png}
    \caption{Example interpretability visualization for a Norwegian residential scene. The model correctly identifies ``Norway'' as the top child concept and ``road\_sign'', ``script\_latin'', ``landscape\_arctic'' as parent concepts. The attention heatmap shows focus on road markings and architectural features. Gate value (0.483) indicates balanced concept-image fusion.}
