...
Figure~\ref{fig:geocells} visualizes the resulting tessellation, showing how cell density adapts to data availability. The Voronoi diagram illustrates coverage, while the latitude distribution confirms a slightly higher concentration in northern hemisphere regions.
...
The first training stage adapts the StreetCLIP vision encoder to our specific domain through multi-objective contrastive learning. While StreetCLIP provides strong geographic priors from pretraining on street view imagery, we find that partial fine-tuning improves downstream concept learning and geolocation accuracy. We unfreeze the top two transformer layers of the vision encoder while keeping lower layers frozen to preserve general visual representations. The model architecture includes several key components for alignment:

\textbf{GPS Adapter.} The GeoCLIP location encoder produces 512-dimensional GPS embeddings, which we project to 768 dimensions to match the StreetCLIP image feature space for contrastive learning.
...
\end{equation}
where $\gamma = 2.0$ controls the focus on hard examples. A hierarchical consistency loss ensures that child predictions aggregate correctly to parent predictions through KL divergence between the expected and predicted parent distributions:
\begin{equation}
...
\end{equation}
where $M_{\text{hier}} \in \mathbb{R}^{k_c \times k_p}$ is the child-to-parent mapping matrix. Stage 1 trains for 50 epochs with batch size 256 using precomputed image embeddings for efficiency, learning rate $3 \times 10^{-4}$, and AdamW optimizer \parencite{loshchilov2017adamw}.
...
The Stage 2 model receives frozen concept embeddings $z \in \mathbb{R}^{512}$ from Stage 1 (via the frozen concept bottleneck) and frozen patch tokens $P \in \mathbb{R}^{576 \times 1024}$ from the image encoder. The architecture supports three ablation modes to understand component contributions:
\begin{enumerate}
    \setlength{\itemsep}{0pt}
    \item \textbf{Both}: Learned gated fusion of concepts and spatial information (default)
    \item \textbf{Concept-only}: Predictions from concept embedding alone
    \item \textbf{Image-only}: Predictions from pooled patch tokens alone
\end{enumerate}
...
\textbf{Learned Gating Mechanism.} The key innovation is how we combine the concept embedding with the cross-attention output. Rather than simple concatenation or addition, we use a learned gate that controls the contribution of each information source \parencite{srivastava2015highway, arevalo2017gated}:
\begin{align}
...
\end{align}
where $\sigma$ is the sigmoid function, $\odot$ is element-wise multiplication, and $g$ is a 512-dimensional gate vector. Following feature-wise gating approaches \parencite{perez2018film}, each dimension independently learns to balance concept versus spatial information. The gate values provide interpretability:
\begin{enumerate}
    \setlength{\itemsep}{0pt}
...
\begin{enumerate}
    \setlength{\itemsep}{0pt}
    \item \textbf{Cell Head}: Softmax classification over 1,048 geocells
    \item \textbf{Offset Head}: MSE regression predicting 3D Cartesian offset from cell center
\end{enumerate}
...
\textbf{Gate Value Analysis.} Figure~\ref{fig:gate_distribution} shows the 90th percentile of gate values during validation across training epochs, comparing the vanilla and finetuned model variants.
...
Both model variants converge to high 90th percentile gate values (0.83--0.86), indicating that concept information dominates the upper dimensions of the embedding space. The vanilla model shows slightly higher gate values than the finetuned model, suggesting that without Stage 0 pretraining, the model relies more heavily on semantic concepts. The finetuned model, having learned better spatial representations through contrastive pretraining, can leverage both information sources more effectively, resulting in a more balanced fusion.
...
% \textbf{Gated Fusion Interpretability.} The learned gating mechanism in Stage 2 provides additional interpretability beyond concept activations. Figure~\ref{fig:gate_distribution} shows the 90th percentile of gate values over training epochs, comparing vanilla and finetuned variants. Both models converge to high gate values (0.83--0.86), indicating that concept information dominates the upper dimensions of the embedding space. The vanilla model shows slightly higher gate values than the finetuned model, suggesting that without Stage 0 pretraining, the default StreetCLIP encoder produces less informative spatial features, forcing the model to compensate by relying more heavily on semantic concepts. The finetuned model, having learned better spatial representations through contrastive pretraining, can leverage spatial information more effectively, resulting in lower gate values and a more balanced fusion between concepts and spatial features.

\textbf{Stage 0 Contrastive Pretraining.} Stage 0 contrastive pretraining provides clear benefits on both in-distribution and out-of-distribution data. While not substantially increasing the concept prediction accuracies, the finetuned model variant reduces median error from 133 km to 126 km on test data and from 392 km to 350 km on the GeoGuessr dataset. This consistent improvement across evaluation settings demonstrates that contrastive pretraining learns broadly transferable representations, validating the value of domain-specific pretraining for geolocation tasks. The GPS adapter (512d$\to$768d) is critical for aligning GeoCLIP location embeddings with StreetCLIP image features in the shared embedding space.
...
Future directions include expanding the dataset for better concept representations and more finegrained geolocation capabilities and improving concept vocabulary through automatic discovery.
