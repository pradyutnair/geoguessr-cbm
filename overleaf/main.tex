...
\begin{figure}[t!]
    \centering
    \includegraphics[width=1.1\textwidth]{arch3.png}
    \caption{Three-stage architecture for concept-aware geolocation. \textbf{Stage 0} (top) performs domain contrastive pretraining to align image, GPS, and concept embeddings, with a GPS adapter ($512\text{d} \to 768\text{d}$) projecting GeoCLIP location features to match StreetCLIP image features. \textbf{Stage 1} (middle) learns hierarchical concept classification through text-anchored prototypes with cosine similarity. \textbf{Stage 2} (bottom) predicts geographic coordinates via learned gated fusion: concept embeddings query image patches through cross-attention to extract spatial context, then a learned gate balances concept versus spatial information for each dimension.}
